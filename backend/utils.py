...
def engineer_features(payload: Any) -> dict[str, Any]:
    """
    ...
    cholesterol = (
        payload.cholesterol
        if payload.cholesterol is not None
        else POPULATION_DEFAULTS["cholesterol"]
    )
    ...
    triglycerides = (
        ...
        if payload.triglycerides is not None
        else POPULATION_DEFAULTS["triglycerides"]
    )
...
def compute_contributing_factors(payload, resolved: dict, db_prob: int, hd_prob: int) -> list[dict]:
    ...
    if payload.familyDiabetes == "Both":
        factors.append(
            {
                "name": "Genetics: High Family Diabetes Predisposition",
                "weight": 30,
                "positive": True,
            }
        )
    ...
    if resolved["hba1c"] >= 6.5:
        factors.append(
            {
                "name": f"Clinical HbA1c Lab Indicator ({resolved['hba1c']}%)",
                "weight": 42,
                "positive": True,
            }
        )
        ...
        factors.append(
            {"name": f"Elevated HbA1c ({resolved['hba1c']}%)", "weight": 22, "positive": True}
        )

    if resolved["glucose"] >= 126:
        factors.append(
            {
                "name": f"High Fasting Blood Glucose ({resolved['glucose']} mg/dL)",
                "weight": 36,
                "positive": True,
            }
        )
        ...
        factors.append(
            {
                "name": f"Elevated Fasting Glucose ({resolved['glucose']} mg/dL)",
                "weight": 18,
                "positive": True,
            }
        )
    ...
    if bmi >= 30 or "obesity" in combs:
        factors.append(
            {"name": "Comorbidity: Obesity Class (BMI ≥ 30)", "weight": 24, "positive": True}
        )
    elif bmi >= 25:
    ...
    if "polyuria" in syms:
        factors.append(
            {"name": "Symptom: Frequent Urination (Polyuria)", "weight": 18, "positive": True}
        )
    if "polydipsia" in syms:
        factors.append(
            {"name": "Symptom: Excessive Thirst (Polydipsia)", "weight": 18, "positive": True}
        )
    if "numbness" in syms:
        factors.append(
            {
                "name": "Symptom: Extremity Numbness (Neuropathy Sign)",
                "weight": 15,
                "positive": True,
            }
        )
    ...
    if payload.physicalActivity == "Low":
        factors.append(
            {"name": "Physical: Sedentary Lifestyle Profile", "weight": 14, "positive": True}
        )
    ...
    if payload.dietQuality >= 8:
        factors.append(
            {"name": "Nutrition: High Quality Whole-Food Diet", "weight": 7, "positive": False}
        )
    elif payload.dietQuality <= 4:
        factors.append(
            {"name": "Diet: Processed Sugars / Refined Carbs", "weight": 15, "positive": True}
        )

    # ── Cardiovascular factors ──
    if payload.familyHeart == "Both":
        factors.append(
            {"name": "Genetics: High Cardiovascular Predisposition", "weight": 26, "positive": True}
        )
    elif payload.familyHeart == "One":
        factors.append(
            {"name": "Family History of Cardiovascular Disease", "weight": 12, "positive": True}
        )

    if "hypertension" in combs:
        factors.append(
            {
                "name": "Comorbidity: Clinically Diagnosed Hypertension",
                "weight": 20,
                "positive": True,
            }
        )
    if "hyperlipidemia" in combs:
        factors.append(
            {
                "name": "Comorbidity: Clinically Diagnosed Hyperlipidemia",
                "weight": 15,
                "positive": True,
            }
        )
    if "kidney_disease" in combs:
        factors.append(
            {"name": "Comorbidity: Chronic Kidney Disease", "weight": 12, "positive": True}
        )
    ...
    if sys_bp >= 180 or dia_bp >= 120:
        factors.append(
            {
                "name": f"Vitals: Hypertensive Crisis BP ({int(sys_bp)}/{int(dia_bp)})",
                "weight": 45,
                "positive": True,
            }
        )
        ...
        factors.append(
            {
                "name": f"Vitals: Stage 2 Hypertension BP ({int(sys_bp)}/{int(dia_bp)})",
                "weight": 30,
                "positive": True,
            }
        )
        ...
        factors.append(
            {
                "name": f"Vitals: Stage 1 Hypertension BP ({int(sys_bp)}/{int(dia_bp)})",
                "weight": 16,
                "positive": True,
            }
        )
        ...
        factors.append(
            {
                "name": f"Vitals: Healthy Blood Pressure ({int(sys_bp)}/{int(dia_bp)})",
                "weight": 6,
                "positive": False,
            }
        )
    ...
    if payload.smoking == "Current":
        factors.append(
            {"name": "Habits: Active Tobacco Use Strain", "weight": 28, "positive": True}
        )
    ...
    if resolved["cholesterol"] >= 240:
        factors.append(
            {
                "name": f"Clinical Hypercholesterolemia ({resolved['cholesterol']} mg/dL)",
                "weight": 22,
                "positive": True,
            }
        )
        ...
        factors.append(
            {
                "name": f"Elevated Total Cholesterol ({resolved['cholesterol']} mg/dL)",
                "weight": 10,
                "positive": True,
            }
        )
    ...
    if resolved["ldl"] >= 160:
        factors.append(
            {
                "name": f"High LDL Cholesterol ({resolved['ldl']} mg/dL)",
                "weight": 20,
                "positive": True,
            }
        )
    ...
    if resolved["hdl"] < hdl_limit:
        factors.append(
            {
                "name": f"Cardioprotective Deficit: Low HDL ({resolved['hdl']} mg/dL)",
                "weight": 15,
                "positive": True,
            }
        )
        ...
        factors.append(
            {
                "name": f"Active Cardioprotection: High HDL ({resolved['hdl']} mg/dL)",
                "weight": 8,
                "positive": False,
            }
        )
    ...
    if resolved["triglycerides"] >= 200:
        factors.append(
            {
                "name": f"Elevated Triglycerides ({resolved['triglycerides']} mg/dL)",
                "weight": 12,
                "positive": True,
            }
        )
    ...
    if "chest_pain" in syms:
        factors.append(
            {"name": "Symptom: Acute Chest Pain (Angina Marker)", "weight": 35, "positive": True}
        )
    if "dyspnea" in syms:
        factors.append(
            {"name": "Symptom: Shortness of Breath (Dyspnea)", "weight": 20, "positive": True}
        )

    if payload.stressLevel > 6:
        factors.append(
            {
                "name": "Stress: Elevated Psychological Index",
    ...
    if payload.alcohol == "Heavy":
        factors.append(
            {"name": "Alcohol: Heavy Consumption Workload", "weight": 12, "positive": True}
        )
    if payload.sleepHours < 6:
        factors.append(
            {"name": "Sleep Deprivation: < 6 Hours Average", "weight": 10, "positive": True}
        )

    # Deduplicate by name, sort by weight descending, return top 5
    seen = set()
    unique = []
    for f in sorted(factors, key=lambda x: x["weight"], reverse=True):  # type: ignore
