...
def _run_tesseract_ocr(file_bytes: bytes) -> str | None:
    ...
    try:
        ...
        log.info(
            "📝 OCR text extracted successfully using Tesseract (Length: %d chars).", len(text)
        )
        return str(text)
...
def generate_mock_report_data(file_name: str) -> dict[str, Any]:
    ...
    if "diabetic" in normalized_name or "glucose" in normalized_name or "sugar" in normalized_name:
        vitals.update({"glucose": 168.0, "hba1c": 7.6, "systolic": 132.0, "diastolic": 82.0})
    elif (
        "cardio" in normalized_name
        or "heart" in normalized_name
        or "lipid" in normalized_name
        or "cholesterol" in normalized_name
    ):
        vitals.update(
            {
                "cholesterol": 265.0,
        ...
        vitals.update({"systolic": 175.0, "diastolic": 105.0, "triglycerides": 180.0})

    return vitals


def extract_vitals_from_report(
    file_bytes: bytes,
    file_name: str,
    provider: str = "gemini",
