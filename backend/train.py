...
def load_uci_heart() -> pd.DataFrame:
    ...
    try:
        ...
        thalach = df.get(
            "thalach", df.get("max_heart_rate", df.get("maximum_heart_rate_achieved", 150))
        )
...
def build_ensemble(use_smote: bool = True):
    ...
    gb = GradientBoostingClassifier(
        n_estimators=200, learning_rate=0.05, max_depth=4, subsample=0.8, random_state=RANDOM_STATE
    )
...
def train_and_evaluate(X: np.ndarray, y: np.ndarray, label: str):
    ...
    log.info("\n═══ Training: %s ═══", label)
    log.info(
        "  Dataset size : %d rows | Positive class: %d (%.1f%%)", len(y), y.sum(), 100 * y.mean()
    )
